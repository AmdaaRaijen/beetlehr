...
const getAttendanceOverviewData = debounce(async (page) => {
    axios
        ...
        .then((res) => {
            let present = attendanceOverview.value.find(
                (e) => e.type === "present"
            );
            let absent = attendanceOverview.value.find(
                (e) => e.type === "absent"
            );
            ...
            let leave = attendanceOverview.value.find(
                (e) => e.type === "leave"
            );
            let holiday = attendanceOverview.value.find(
                (e) => e.type === "holiday"
            );
...
<template>
    ...
    <div class="bg-white shadow-lg rounded-md mb-8 p-4">
        <div class="mb-5 flex justify-between items-center">
            <h1 class="text-xl text-slate-800 font-semibold">
                Attendance Overview
            </h1>
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        ...
        <section class="py-6 px-4">
            ...
            >
                <tr
                    v-if="
                        attendanceQuery.length === 0 && !attendanceListLoading
                    "
                >
                    <td
                ...
                >
                    ...
                    >
                        ...
                        >
                            <VHoliday
                                v-if="attendanceData.status === 'holiday'"
                            />
                            <VPresent
                                v-else-if="attendanceData.status === 'present'"
                                @click="
                                    (openModalForm = true),
                                        (itemSelected = attendanceData)
                                "
                            />
                            <VLate
                            ...
                            />
                            <VAbsent
                                v-else-if="attendanceData.status === 'absent'"
                            />
                            <VClockoutEarly
                                v-else-if="
                                    attendanceData.status === 'clockout_early'
                            ...
                            />
                            <VLeave
                                v-else-if="attendanceData.status === 'leave'"
                            />
                            <VUnassign
                                v-else-if="
                                    attendanceData.status === 'unassigned'
                                "
                            />
                        </div>
                    </td>
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        ...
        <section class="py-6 px-4">
            ...
            >
                <tr
                    v-if="
                        attendanceRecapQuery.length === 0 &&
                        !attendanceRecapLoading
                    "
                >
                    <td
                        class="overflow-hidden my-2"
                ...
                <tr
                    v-for="(data, index) in attendanceRecapQuery"
                    :key="index"
                    v-else
                >
                    <td
                        class="px-4 whitespace-nowrap h-16 fixed-left capitalize"
                    >
