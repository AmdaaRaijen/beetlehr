...
const submit = async () => {
    if (form.value.start_time && form.value.end_time) {
        form.value.start_time =
            form.value.start_time.hours + ":" + form.value.start_time.minutes;
        form.value.end_time =
            form.value.end_time.hours + ":" + form.value.end_time.minutes;
    }
...
const handleShiftType = () => {
    if (form.value.start_time && form.value.end_time) {
        const start_time =
            form.value.start_time.hours +
            ":" +
            form.value.start_time.minutes +
            ":0";
        const end_time =
            form.value.end_time.hours +
            ":" +
            form.value.end_time.minutes +
            ":0";
        form.value.is_night_shift =
            Date.parse("01/01/2011 " + end_time) <
            Date.parse("01/01/2011 " + start_time)
                ? true
                : false;
        form.value.shift_type = form.value.is_night_shift
            ? "Night Shift"
            : "Normal Shift";
    }
};
...
const createData = async () => {
    ...
    axios
        .post(route("attendance.shift.create"), form.value)
        .then((res) => {
            // console.log(res)
            emit("close");
            emit("successSubmit");
...
const updateData = async () => {
    ...
    axios
        .put(
            route("attendance.shift.update", { id: props.data.id }),
            form.value
        )
        .then((res) => {
            emit("close");
...
<template>
    <VDialog
        ...
        :title="updateAction ? 'Update Shift' : 'Add Shift'"
        @opened="openForm"
        @closed="closeForm"
        size="xl"
    >
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                ...
                <VInput
                    ...
                    :errorMessage="formError.shift_type"
                    @update:modelValue="formError.shift_type = ''"
                    disabled
                />
                ...
                <div>
                    ...
                    <Datepicker
                        ...
                        @update:modelValue="handleShiftType()"
                        time-picker
                        position="left"
                        :clearable="true"
                        placeholder="00:00"
